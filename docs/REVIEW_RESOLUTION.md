...
最新进度（2026-09-25）：会话切换时旧状态短暂显示已完成本地修复。目标 runtime 的项目/会话/职能确认前不恢复旧缓存，历史加载期间隐藏状态条，后台状态继续保留。旧代码 8 个定向场景中 6 个失败，修复后 8/8、真实聊天组件加载态检查、完整 Harness 366 例 × 3 均通过；新隔离原生批次 `d7-wBZ41o` 的加载中间态、冷恢复和 A/B 切换也已观察通过，真实模型调用 0。尚未提交推送或执行本批远程 CI，详见 [会话状态投影验收](REVIEW_SESSION_STATUS_ACCEPTANCE.md)。
