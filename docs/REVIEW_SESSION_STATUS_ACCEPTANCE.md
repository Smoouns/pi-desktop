...
日期：2026-09-25。基线 `ac4f942a88e34060eec3adf6022c83bcc41e5e79`；以下记录提交前的本地修复与验收，原始报告中的 HEAD/dirty 不追改。修复已提交推送为 `b253286804196a37b58492db72d10311d7f10830`，固定提交的远程 CI 结果见末节。上一批记录见 [E 固定版本验收](REVIEW_E_FIXED_ACCEPTANCE.md)，其中旧状态闪现的失败观察保留不改写。
...
- 以上本地证据不替代固定提交的远程 CI，也不关闭 E 中仍保留的真实模型摘要质量、兼容计量与真实小说长程质量边界。

## 提交与固定版本 CI

修复提交 `b253286804196a37b58492db72d10311d7f10830` 已推送到 `origin/main`，包含 11 个修复、测试及验收文档文件。三份根目录用户规划/研究原稿、`artifacts/`、认证配置与真实小说未纳入提交。

[CI 36148840138](https://github.com/Smoouns/pi-desktop/actions/runs/36148840138) 于 `2026-09-25T15:10:58Z` 完成，第一次运行即全部通过。已回读 run 的完整 `head_sha`、四个 job 的步骤状态和完成日志，而非只依据绿色图标。

| 任务 | Job ID | 结果 |
| --- | --- | --- |
| Public harness：Ubuntu 22.04 / Node 22 | `108116677118` | 通过 |
| Public harness：Windows / Node 24 | `108116676643` | 通过 |
| TypeScript + Rust：Ubuntu 22.04 | `108116677027` | 通过 |
| TypeScript + Rust：Windows | `108116676921` | 通过 |

- 两平台均实际输出 Harness 366 例 × 3、`deterministic=true / failures=false`；新增 `SESSION-BIND-01` 至 `08` 各执行三次并全部通过，已包含在 366 例中，不额外累计。
- 两平台长程回归、完整生产生命周期 13 组 / 24 个子进程阶段、发送层 15 例、小说域、评测自检、Pilot 及完整 SDK 套件均执行通过。冻结评测的预期能力负例仍按原合同核验，不当作额外产品成功数。
- Windows 两次 Pilot journal 均为 11 项、`unsupported=[]`，短路径回归实际执行；Linux 为 10 项、`unsupported=[]`。Windows Rust 控制台两项回归也实际执行通过。
- 跳过项仅为平台条件：Ubuntu 的 Windows 短路径和控制台回归、Windows 的 Linux 系统依赖安装。其余必需步骤没有跳过；最后的证据报告、聊天面板测试、构建与产物上传都通过。
- 固定提交还在本机重跑了真实 `ChatView` 浏览器检查：`artifacts/harness/extension-status-ui/check-2H0qKj/summary.json`，两个窗口均通过。该浏览器检查和上面的原生窗口观察是本地证据，未声称 CI 运行了原生 Desktop。
- 本次 CI 为离线/合成回归，没有新增真实模型调用。精简回读记录保存在 `artifacts/harness/ci/session-status-b253286-run36148840138.json`；远程日志与源码归属仍以此处的 run ID 和代码 SHA 为准。

本节结果回填是独立的仅文档 `[skip ci]` 提交，不改生产代码、测试或工作流；CI 结论绑定 `b253286`，不将文档提交冒充又运行了一次 CI。
