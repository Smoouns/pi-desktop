...
assert.ok(["test", "dry-run", "prepare", "live", "recover", "diagnose"].includes(mode), "S4L_MODE_INVALID");
assert.equal(process.argv.length, mode === "live" ? 8 : ["prepare", "recover", "diagnose"].includes(mode) ? 4 : mode === "dry-run" ? (process.argv[3] ? 4 : 3) : 3, "Unexpected S4 arguments");
if (mode === "live") {
...
const work = await mkdtemp(path.join(parent, ".sdk-supervision-live-build-"));
try {
	const bundle = path.join(work, "sdk-supervision-live.mjs"), inputs = {};
	await build({ entryPoints: [path.join(root, "evals/sdk-supervision-live/cli.ts")], outfile: bundle, bundle: true,
	...
	env.PI_ABLATION_BUILD_INPUTS = buildInputs;
	env.PI_S4L_WORK_ROOT = work;
	env.PI_S4L_BUNDLE = bundle;
	...
	if (mode === "live") {
		...
		const authorization = spawnSync(process.execPath, ["--import", pathToFileURL(path.join(root, "scripts/eval-network-guard.mjs")).href,
			bundle, "authorize", ...process.argv.slice(3)], { cwd: root, env, stdio: "ignore", timeout: 15_000, windowsHide: true });
	...
	process.exitCode = await new Promise((resolve) => {
		...
		const timer = setTimeout(stop, mode === "live" ? 2_800_000 : 900_000);
	...
	assert.ok(relative.startsWith(".sdk-supervision-live-build-") && !relative.includes(path.sep), "UNSAFE_PILOT_CLEANUP");
