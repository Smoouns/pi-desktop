...
assert.ok(["test", "dry-run", "prepare", "live", "recover"].includes(mode), "S3L_MODE_INVALID");
assert.equal(process.argv.length, mode === "live" ? 8 : ["prepare", "recover"].includes(mode) ? 4 : mode === "dry-run" ? (process.argv[3] ? 4 : 3) : 3, "Unexpected S3 arguments");
if (mode === "live") {
...
const work = await mkdtemp(path.join(parent, ".sdk-context-live-build-"));
try {
	const bundle = path.join(work, "sdk-context-live.mjs"), inputs = {};
	await build({ entryPoints: [path.join(root, "evals/sdk-context-live/cli.ts")], outfile: bundle, bundle: true,
	...
	env.PI_S3L_WORK_ROOT = work;
	env.PI_S3L_BUNDLE = bundle;
	...
	process.exitCode = await new Promise((resolve) => {
		...
		const timer = setTimeout(stop, mode === "live" ? 1_900_000 : 600_000);
	...
	assert.ok(relative.startsWith(".sdk-context-live-build-") && !relative.includes(path.sep), "UNSAFE_PILOT_CLEANUP");
