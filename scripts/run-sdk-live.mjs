...
assert.ok(["test", "dry-run", "prepare", "live", "recover"].includes(mode), "S2_MODE_INVALID");
assert.equal(process.argv.length, mode === "live" ? 8 : ["prepare", "recover"].includes(mode) ? 4 : 3, "Unexpected pilot arguments");
if (mode === "live") {
...
const work = await mkdtemp(path.join(parent, ".sdk-live-build-"));
try {
	const bundle = path.join(work, "sdk-live.mjs"), inputs = {};
	await build({ entryPoints: [path.join(root, "evals/sdk-live/cli.ts")], outfile: bundle, bundle: true,
	...
	env.PI_SDK_LIVE_WORK_ROOT = work;
	env.PI_SDK_LIVE_BUNDLE = bundle;
	...
	process.exitCode = await new Promise((resolve) => {
		...
		const timer = setTimeout(stop, mode === "live" ? 1_500_000 : 480_000);
	...
	assert.ok(relative.startsWith(".sdk-live-build-") && !relative.includes(path.sep), "UNSAFE_PILOT_CLEANUP");
