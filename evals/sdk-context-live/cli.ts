...
import { contextLiveWorker } from "./session.js";
import { SIMULATIONS, type Simulation } from "./policy.js";
import { runTests } from "../../tests/sdk-context-live/tests.js";
try {
	const mode = process.argv[2];
	if (mode === "worker") await contextLiveWorker();
	else if (mode === "test") console.log(`SDK S3 live tooling tests passed: ${await runTests()}. Zero real model requests.`);
	else if (mode === "dry-run") { const simulation = process.argv[3] ?? "readback"; if (!SIMULATIONS.includes(simulation as Simulation) || simulation === "none") throw new Error("S3L_SIMULATION_INVALID"); console.log(JSON.stringify(await runDry(simulation as Exclude<Simulation, "none">), null, 2)); }
	...
	else throw new Error("S3L_MODE_REJECTED");
} catch (error) { if (process.argv[2] === "test" || process.argv[2] === "dry-run") console.error(error); console.error("SDK_S3_LIVE_REJECTED"); process.exitCode = 1; }
